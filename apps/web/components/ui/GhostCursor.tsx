...
const GhostCursor = ({
  ...
  const FilmGrainShader = useMemo(() => ({
    ...
    `,
    fragmentShader: `
      uniform sampler2D tDiffuse; uniform float iTime; uniform float intensity;
      varying vec2 vUv;
  ...
  function calculateScale(el: HTMLElement) {
    ...
    const base = 600;
    const cur  = Math.min(Math.max(1, r.width), Math.max(1, r.height));
    return Math.max(0.5, Math.min(2.0, cur / base));
  }
  ...
  useEffect(() => {
    ...
    const resize = () => {
      if (!active) return;
      // For full-page fixed overlays, use window dimensions directly
      const rect = host.getBoundingClientRect();
      const cssW = Math.floor(rect.width  > 0 ? rect.width  : window.innerWidth);
      const cssH = Math.floor(rect.height > 0 ? rect.height : window.innerHeight);
    ...
    // Also listen to window resize for full-page mode
    const onWinResize = () => { if (active) resize(); };
    ...
    // Use the renderer's canvas dimensions for coordinate mapping
    // when in full-page mode — listen on window so pointer-events:none wrappers don't block
    ...
    return () => {
      ...
      if (!prevPos || prevPos === 'static') parent.style.position = prevPos;
    };
